document.addEventListener('DOMContentLoaded', () => {
    ...
    function createOrganicTunnel(left, right, tunnelLength, colorHex, status) {
        console.log("Creating tunnel:", {left, right, tunnelLength, colorHex, status});
        if(tunnelMesh) {
            scene.remove(tunnelMesh);
            console.log("Removed old tunnel mesh");
        }
        if(labelGroup) {
            labelGroup.clear();
        } else {
        ...
        // Branch Tunnel (As seen in reference image)
        if (tunnelLength > 100) {
            createTunnelPart(left*0.7, right*0.7, tunnelLength*0.6, -tunnelLength/3 + 100, centerOffset + 20, Math.PI / 6);
        }
    ...
    function animate() {
        ...
        TWEEN.update(); // Update camera glides
